...
impl<'a> ServerHelloParser<'a> {
    ...
    pub fn parse_server_response(&mut self, length: u16) {
        ...
        while self.offset < length as usize {
            ...
            match handshake_type {
                ...
                0x0c => info!(
                    "    Handshake Type: {:02X} (Server Key Exchange)",
                    handshake_type
                ),
                0x0e => info!(
                    "    Handshake Type: {:02X} (Server Hello Done)",
                    handshake_type
                ),
    ...
    fn parse_server_hello(&mut self) {
        ...
        trace!(
            "      Cipher Suite: {:04X} (Cipher Suite Code)",
            cipher_suite
        );
        self.offset += 2;
        ...
        trace!(
            "      Compression Method: {:02X} (Compression Method)",
            compression_method
        );
        self.offset += 1;
        // 解析 Extensions
        if self.offset < self.data.len() {
            let extensions_length =
                u16::from_be_bytes([self.data[self.offset], self.data[self.offset + 1]]);
    ...
    fn parse_extensions(&mut self, length: usize) {
        ...
        while self.offset < end {
            ...
            self.offset += 2;
            let extension_name = TLS_EXTENSIONS
                .get(&extension_type)
                .map_or("Unknown", |&name| name);
            trace!(
                "      Extension: {:<25} (Type: {:<04X}, Length: {:04X} ({:<3} bytes))",
                extension_name,
                extension_type,
                extension_length,
                extension_length
...
pub fn parse_server_response(data: &[u8], check_enable: bool) -> String {
    let mut parser = ServerHelloParser::new(data);
    let record_layers = parser.parse_tls_record_layers();
    parser.offset = 0;
    let mut result = String::from("Unknown"); // 改为String类型
    if !check_enable {
        ...
        match parser.data[parser.offset + 5] {
            0x02 => {
                ...
                if &parser.data[parser.offset + 11..parser.offset + 43] == RETRY_REQUEST_RANDOM {
                    ...
                    result = String::from("ServerRetryRequest");
                } else {
                    info!(target: "handshake_record", "{}","(ServerHello)\n");
                    result = String::from("ServerHello");
                }
            }
            0x0b => {
                info!(
                    "{}",
                    format!("Handshake Type: {:02X} (Certificate)\n", handshake_type)
                );
                result = String::from("Certificate");
            }
            0x0c => {
                info!(
                    "{}",
                ...
                result = String::from("ServerKeyExchange");
            }
            0x0e => {
                info!(
                    "{}",
                ...
                result = String::from("ServerHelloDone");
            }
            _ => {
                debug!(
                    target: "handshake_record",
                    "{}",
                    format!("Handshake Type: {:02X} (Unknown)\n", handshake_type)
                );
                result = format!("{:02X} Unknown", handshake_type);
            }
        }
    }
    for (index, record) in record_layers.into_iter().enumerate() {
        ...
        trace!(
            "  Content Type: {:02X} ({})",
            record.content_type,
            parser.content_type_to_string(record.content_type)
        );
        trace!(
            "  Version: {:02X} {:02X} (TLS Version)",
            record.version.0,
