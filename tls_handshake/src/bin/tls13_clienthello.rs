use colored::*;
use mio::{net::TcpStream as MioTcpStream, Events, Poll, Token};
use rustls::{ClientConfig, ClientConnection, Stream};
use std::fs::{self};
use std::io::{ErrorKind, Read, Write};
use std::{error::Error, fs::File, path::Path, sync::Arc, thread::sleep, time::Duration};
...
use tls_handshake::{
    clienthello_mutator, clienthello_parser, network_connect, server_response, terminal,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut connection_second = ClientConnection::new(
        config.clone(),
        terminal::get_server_name(&matches).try_into()?,
    )?;
    ...
    let mut config_builder = Config::builder()
        .appender(
            Appender::builder()
                ...
                .build(
                    "clienthello_mutator",
                    Box::new(clienthello_mutator_appender),
                ),
        )
        .appender(
            Appender::builder()
        ...
        .appender(
            Appender::builder()
                ...
                .build(
                    "handshake_log",
                    Box::new(default_client_server_handshake_record),
                ),
        )
        .logger(
            Logger::builder()
        ...
        config_builder = config_builder
            ...
            .appender(
                Appender::builder()
                    .filter(Box::new(ThresholdFilter::new(LevelFilter::Trace)))
                    .build(
                        "clienthello_parser_json",
                        Box::new(clienthello_parser_json_appender),
                    ),
            )
            .logger(
                Logger::builder()
        ...
        config_builder = config_builder
            .appender(
                Appender::builder()
                    .filter(Box::new(ThresholdFilter::new(LevelFilter::Trace)))
                    .build(
                        "clienthello_mutator_parser",
                        Box::new(clienthello_mutator_parser_appender),
                    ),
            )
            .logger(
                Logger::builder()
            ...
            config_builder = config_builder
                .appender(
                    Appender::builder()
                        .filter(Box::new(ThresholdFilter::new(LevelFilter::Trace)))
                        .build(
                            "server_response_parser",
                            Box::new(server_response_parser_appender),
                        ),
                )
                .logger(
                    Logger::builder()
...
fn perform_first_handshake(
    ...
    match tls.read_to_end(&mut plaintext) {
        ...
        Err(ref err) if err.kind() == ErrorKind::UnexpectedEof => {
            ...
            // 分割响应头和主体，以双换行（\r\n\r\n 或 \n\n）为界
            if let Some((headers, _)) = response
                .split_once("\r\n\r\n")
...
fn mutate_client_hello_if_enabled(
    ...
    for single_mutation in mutation_store {
        mutated_clienthello_tmp_store = clienthello_mutator::preferred_mutate_client_hello(
...
fn send_client_hello_if_test_env(
    ...
    for single_clienthello in client_hello {
        // 重新建立连接
        let mut stream = match network_connect::connect_to_server(
            server_ip,
            port,
            terminal::get_easy_read(matches),
        ) {
            Ok(stream) => stream,
            Err(e) => {
        ...
        match wait_for_server_response(&mut poll, token, &mut stream) {
            Ok(server_response) => {
                let parse_result = server_response::parse_server_response(
                    &server_response,
                    terminal::get_check_parse_sh(&matches),
                );
                info!(target: "handshake_record","{}", format!("Parse result: {}\n\n",parse_result));
                // info!(target: "handshake_record", "Parse result\n\n");
            }
            Err(e) => {
...
fn read_mutation_source<'a>(
    ...
    for result in reader.records() {
        ...
        let message = clienthello_mutator::parse_message(
            &record[headers.iter().position(|h| h == "message").unwrap()],
        )?;
        let field = record[headers.iter().position(|h| h == "field").unwrap()].to_string();
        let action = clienthello_mutator::parse_action(
            &record[headers.iter().position(|h| h == "action").unwrap()],
        )?;
        let relative_position =
            record[headers.iter().position(|h| h == "relative_to").unwrap()].to_string();
        ...
        test_mutation_vec.push(clienthello_mutator::create_test_mutation(
            message,
            field,
            action,
            relative_position,
        ...
        for event in &events {
            if event.token() == token && event.is_readable() {
                ...
                match stream.read(&mut buffer) {
                    ...
                    Ok(_) => {
                        warn!(target: "handshake_record", "Received 0 bytes. Continuing to wait...")
                    }
                    Err(ref e) if e.kind() == std::io::ErrorKind::WouldBlock => {
