...
use windows::Win32::Foundation::{
    CloseHandle, GetLastError, BOOLEAN, HANDLE, WAIT_FAILED, WAIT_OBJECT_0, WAIT_TIMEOUT,
};
use windows::Win32::System::Threading::{
    CreateEventA, RegisterWaitForSingleObject, UnregisterWait, WaitForSingleObject, INFINITE,
    WT_EXECUTEONLYONCE,
};
// use windows::Win32::System::WindowsProgramming::INFINITE;
use crate::windows_ping::{PingHandle, MAX_UDP_PACKET};
...
fn register_event(waker_address: *const c_void) -> (HANDLE, HANDLE) {
    ...
    unsafe {
        let result = RegisterWaitForSingleObject(
            &mut event_registration,
            ping_event,
            Some(reply_callback),
            Some(waker_address),
            INFINITE,
...
impl<'a> FutureEchoReplyAsyncState<'a> {
    ...
    fn start(&mut self) -> Option<Poll<PingApiOutput>> {
        (self.ping_event, self.event_registration) =
            register_event(self.waker_address() as *const c_void);

        let raw_reply = windows_ping::echo(
            self.handle.icmp(),
            *self.handle.icmp_handle(),
            Some(self.ping_event),
            self.data.as_ref(),
            self.mut_reply_buffer(),
            self.timeout,
            self.options,
        )
...
impl<'a> Future for FutureEchoReplyAsyncState<'a> {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        if async_state.ping_event.is_invalid() {
            if let Some(result) = async_state.start() {
                return result;
            }
        }
        ...
        match ping_state {
            ...
            WAIT_OBJECT_0 => Poll::Ready(
                async_state
                    .handle
                    .icmp()
                    .create_raw_reply(async_state.mut_reply_buffer())
                    .into(),
            ),
            WAIT_FAILED => Poll::Ready(Err(PingError::OsError(
                unsafe { GetLastError().0 },
                "Wait event failed".to_string(),
            ))),
            _ => Poll::Ready(Err(PingError::OsError(
                ping_state.0,
                "Unexpected return code!".to_string(),
            ))),
        }
    }
}
