...
use windows::Win32::NetworkManagement::IpHelper::{
    Icmp6CreateFile, IcmpCloseHandle, IcmpCreateFile, IcmpHandle, IP_OPTION_INFORMATION,
    IP_STATUS_BASE,
};
...
impl Into<PingApiOutput> for PingRawReply {
    fn into(self) -> PingApiOutput {
        parse_raw_reply_status(self.status).map(|_| PingReply {
...
fn validate_data_buffer(data: &[u8]) -> Result<&[u8]> {
    if data.len() > MAX_BUFFER_SIZE {
        Err(PingError::DataSizeTooBig(MAX_BUFFER_SIZE))
    } else {
        Ok(data)
    }
}
...
fn initialize_icmp_handle(addr: &IpAddr) -> Result<PingHandle> {
    unsafe {
        ...
        handle
            ...
            .map_err(ping_reply_error)
    }
}

const DONT_FRAGMENT_FLAG: u8 = 2;
pub(crate) fn echo(
    destination: &dyn IcmpEcho,
    handle: IcmpHandle,
    event: Option<HANDLE>,
    buffer: &[u8],
    reply_buffer: *mut u8,
    ...
    let mut options_data: [u8; 4] = [0; 4];
    options_data[0] = 68; // Timestamp选项类型
    options_data[1] = 4; // 长度
    options_data[2] = 1; // 指针
    options_data[3] = 0; // 溢出和标志
    ...
    let error = destination.send(
        handle,
        event,
        request_data,
        buffer.len() as u16,
        ip_options_ptr,
        reply_buffer as *mut c_void,
        MAX_UDP_PACKET as u32,
    ...
    if error == 0 {
        ...
        if win_err == ERROR_IO_PENDING {
            ...
            Err(ping_reply_error(win_err.0))
        }
    } else {
        Ok(reply_buffer)
    }
...
fn parse_raw_reply_status(status: u32) -> Result<()> {
    if status as IpStatus::Type == IpStatus::Success {
        Ok(())
    } else {
...
fn ping_reply_error(status_code: u32) -> PingError {
    if status_code < IP_STATUS_BASE {
        ...
        if status_code == IpStatus::TimedOut {
            PingError::TimedOut
        } else {
